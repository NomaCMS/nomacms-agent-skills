...
Auth methods are called directly on the client (not namespaced). Configure the client with `projectUserAuth` — see `nomacms-sdk-setup` for client creation.
...
For framework-agnostic guardrails and must-pass checks, read `nomacms-auth-contract` first.
...
- Token storage adapter setup: `nomacms-sdk-setup`
- Next.js patterns (including server forwarding): `nomacms-nextjs`
- Contract and integration checklist: `nomacms-auth-contract`
- Next.js auth guardrails and review flow: `nomacms-nextjs-auth`, `nomacms-auth-review`
