...
Payload shapes for collections and fields usually align with **`nomacms-collections-fields`** and its [reference.md](../nomacms-collections-fields/reference.md) (for example `is_singleton`, field `type`, `slug`, `required`).
...
If body copy is long or formatted, use a `richtext` field and respect **`options.editor.outputFormat`** as documented in **`nomacms-collections-fields`**.
