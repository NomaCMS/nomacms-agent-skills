---
name: nomacms-mcp-content-structure
description: >-
...
1. **Infer schema from the page plan**  
   Map each section to collections: singletons for one-off page copy (hero, footer blurb, global nav labels) vs repeating rows (feature cards, testimonials, footer links). Choose **stable slugs** (kebab-case) the app will query. **Do not** add custom “published at” date fields for entries that only need a public timestamp — Noma already exposes **`published_at`** on every entry; duplicating it in **`fields`** is a common agent mistake (see **`nomacms-content`** and **`nomacms-collections-fields`**).
...
2. **Use the Noma MCP before finishing the UI**  
   ...
   - For **multilingual** apps, define project **locales** first (`add_project_locale` per code, optionally `set_default_project_locale`), or use the **REST** routes in **`nomacms-sdk-setup`** — the content API’s `locale` parameter only works for locales already on the project.  
...
3. **Then implement the frontend**  
   Wire the stack the user is using: follow **`nomacms-nextjs`**, **`nomacms-nuxt`**, or **`nomacms-astro`** when applicable; otherwise use **`nomacms-sdk-setup`** + **`nomacms-content`** with `@nomacms/js-sdk`. Fetch by the **exact collection slugs** you created; types or field access must match the schema you defined. For **`is_singleton: true`** collections, **`content.list(slug)`** returns **one object**, not a paginated list — see **`nomacms-content`** (list response matrix). Do not use **`paginate`** for singletons.

4. **If the Noma MCP is missing or fails**  
   Say so clearly. Fall back to **`nomacms-collections-fields`** with a server-only `apiKey` (one-off script, API route, Nitro/server handler, server action, etc.) — same schema plan, SDK instead of MCP.
...
- Field types, `collections.*` / `fields.*` payloads: **`nomacms-collections-fields`**
- Listing and rendering entries: **`nomacms-content`**
- Framework guides: **`nomacms-nextjs`**, **`nomacms-nuxt`**, **`nomacms-astro`**
