...
Content requests use a **`locale`** query/body field only for locale codes that exist on the project. **Configure locales on the project** (dashboard **Localization**, or **`POST /api/project/locales`** with an **`admin`** API key — see **`nomacms-sdk-setup`**) before building a multi-language Next.js or other app.
