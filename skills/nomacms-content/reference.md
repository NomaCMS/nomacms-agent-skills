...
**Agent rule (timestamps):** each entry includes **`published_at`** at the entry root. Prefer **`entry.published_at`** and `sort` on **`published_at`** / **`created_at`** for “when did this go live?” — do **not** introduce a custom field that only mirrors that unless the product needs a different semantic (documented in **`nomacms-content`** SKILL).
